    ...
    public class TutorRepository
    {
        ...
        public List<TutorCard> GetTopTutors()
        {
            ...
            using (SqlConnection connectDB = new SqlConnection(_connectionString))
            {
                ...
                // Fetch modules for each tutor
                if (tutors.Any())
                {
                    string modulesSql = @"
                        SELECT tm.tutorId, tm.moduleCode
                        FROM tutorModules tm
                        WHERE tm.tutorId IN ({0})";

                    var tutorIds = tutors.Select(t => t.TutorId).ToList();
                    var parameters = new List<SqlParameter>();
                    var tutorIdParams = string.Join(",", tutorIds.Select((id, index) =>
                    {
                        var paramName = $"@tutorId{index}";
                        parameters.Add(new SqlParameter(paramName, id));
                        return paramName;
                    }));

                    modulesSql = string.Format(modulesSql, tutorIdParams);

                    using (SqlCommand modulesCmd = new SqlCommand(modulesSql, connectDB))
                    {
                        modulesCmd.Parameters.AddRange(parameters.ToArray());
                        using (SqlDataReader reader = modulesCmd.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                int tutorId = reader.GetInt32(0);
                                string moduleCode = reader.GetString(1);
                                var tutor = tutors.FirstOrDefault(t => t.TutorId == tutorId);
                                if (tutor != null)
                                {
                                    tutor.Modules.Add(moduleCode);
                                }
                            }
                        }
                    }
                ...
                using (SqlCommand command = new SqlCommand(query, connectDB))
                {
                    ...
                    using (SqlDataReader reader = command.ExecuteReader())
                    {
                        if (reader.Read())
                        {
                            ...
                            // Fetch modules for this tutor
                            reader.Close(); // Close the reader before executing another query
                            string modulesSql = @"
                                SELECT tm.moduleCode
                                FROM tutorModules tm
                                WHERE tm.tutorId = @tutorId";

                            using (SqlCommand modulesCmd = new SqlCommand(modulesSql, connectDB))
                            {
                                modulesCmd.Parameters.AddWithValue("@tutorId", tutorId);
                                using (SqlDataReader modulesReader = modulesCmd.ExecuteReader())
                                {
                                    while (modulesReader.Read())
                                    {
                                        tutor.Modules.Add(modulesReader.GetString(0));
                                    }
                                }
                            }
        ...
        public PagedResult<TutorCard> GetTutors(TutorQuery query)
        {
            ...
            using (SqlConnection connectDB = new SqlConnection(_connectionString))
            {
                ...
                var tutorDict = new Dictionary<int, TutorCard>();
                ...
                using (var cmd = new SqlCommand(dataSql, connectDB))
                {
                    ...
                    using (var reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            ...
                            tutorDict.Add(tutor.TutorId, tutor);
                            result.Items.Add(tutor);
                        }
                    }
                }
                ...
                if (tutorDict.Any())
                {
                    ...
                    var tutorIds = tutorDict.Keys.ToList();
                    var parameters = new List<SqlParameter>();
                    var tutorIdParams = string.Join(",", tutorIds.Select((id, index) =>
                    {
                        var paramName = $"@tutorId{index}";
                    ...
                    {
                        modulesCmd.Parameters.AddRange(parameters.ToArray());
                        using (var reader = modulesCmd.ExecuteReader())
                        {
                            while (reader.Read())
                            {
                                int tutorId = reader.GetInt32(0);
                                string moduleCode = reader.GetString(1);
                                if (tutorDict.TryGetValue(tutorId, out var tutor))
                                {
                                    tutor.Modules.Add(moduleCode);
                                }
                            }
